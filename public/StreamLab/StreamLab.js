...
var StreamLabHtml = (function () {
    ...
    StreamLabHtml.prototype.json = function(data) {
        try {
    ...
    StreamLabHtml.prototype.setData = function(response) {
        this.data = this.json(response.data);
    };
    ...
    StreamLabHtml.prototype.getVal = function(id){
        return this.getById(id).value.trim();
    };
    ...
    StreamLabHtml.prototype.extractTemplate = function(id , template , data , customAttr , br) {
        br = br === undefined ? "" : br;
        ...
        for(var i = 0 ; i <= template.length ;i++){
            if (i == 0)
                ...
                tag += customAttr+">"+data+"</"+template[0]+">"+br;
        }
    ...
    StreamLabHtml.prototype.removeTagByDataAttr = function(attr , data ){
        var html = document.querySelectorAll('['+attr+'="'+data+'"]');
        if(html[0] != undefined){
            html[0].outerHTML = "";
            delete html[0];
        }
    ...
    StreamLabHtml.prototype.extractChannel = function(id , response){
        var json = this.json(response).data;
        var z = "";
        for(var i = 0 ;i < json.length ; i++){
            if(this.channelTemplate.length > 0){
                this.extractTemplate(id , this.channelTemplate , "name : " +json[i].name+" <br> online : <span data-channel-online='"+json[i].id+"' id='" +json[i].name+"' >" + json[i].online + "</span>" , "data-channel='"+json[i].id +"' data-channelName='"+json[i].name+"' href='#' onclick='subscribeToChannel(this);return false;' " , '<hr/>');
            }else{
                z = "";
    ...
    StreamLabHtml.prototype.getChannel  = function(channelName , secret  , callback , url){
        secret =  secret === undefined || secret == null ? '' : '&secret='+secret;
        url =  url === undefined  ? '/streamLab/app/get/channel' : url;
        ajax().get(url+'?channelName='+channelName+secret).then(function(response){
            callback(response);
        });
    };
    StreamLabHtml.prototype.createChannel = function(channelName , callback , secret ,  url){
        url = url === undefined ? '/StreamLab/create/Channel/' : url;
        secret = secret === undefined ? '' : '&secret='+secret;
        var thisCalss = this;
        ajax().get(url+'?channelName='+channelName+secret).then(function(response){
            callback = callback === undefined ? '' : callback(thisCalss.json(response));
        });
    };
    StreamLabHtml.prototype.deleteChannel = function(channelName , callback , url){
        url = url === undefined ? '/StreamLab/delete/Channel/' : url;
        var thisCalss = this;
        ajax().get(url+'?channelName='+channelName).then(function(response){
            callback = callback === undefined ? '' : callback(thisCalss.json(response));
        });
    };
...
var StreamLabUser = (function() {
    ...
    StreamLabUser.prototype.userExist  = function(url , id ,  callback){
        var thisCalss = this;
        this.checkDeleteUser(url , id ,  function(response){
            return callback(thisCalss.json(response));
        });
    };
    ...
    StreamLabUser.prototype.getAllUser = function(url , callback , limit , offset , channel){
        ...
        ajax().get(url+limit+offset+channel).then(function(response){
            return callback(thisClass.json(response));
        });
    };
